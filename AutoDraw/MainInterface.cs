        ...
        public void drawSingleStationPicture(Point2d insertSignlePoint, ObjectId fontStyleId, ClassStruct.ProjectInfo projetInfo, ClassStruct.StationPoint StationToBeDraw, List<string> LeftRightStation, List<ClassStruct.EquipePoint> List_EquipmentInfor, List<ClassStruct.ConnectionAndLine> List_ConnectionAndLine,int drawNum)//List<string> connectionDict)
        {
            ...
            using (Transaction trans2 = db2.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    int marginEqui = 210 / (List_EquipmentInfor.Count + 1);
                    //绘图起始点
                    ...
                    List<Point3d> registGouChaoInsertPoint = new List<Point3d>();  //记录插入图块至电缆沟线段的端点
                    ...
                    foreach (KeyValuePair<int,string> LiCheng_Type in lstorder) //根据设备、基站的里程绘图 按
                    {
                        if (StationToBeDraw.distance == LiCheng_Type.Key&& StationToBeDraw.type == LiCheng_Type.Value) //绘制基站
                        {
                            ...
                            if (StationToBeDraw.type == "牵引变电所")  //如果站点是牵引变电所则添加地震点
                            {
                                spaceId.InsertBlockReference("0", "地震传感器", new Point3d(equipeInsertPoint.X + marginEqui * numInsert + 13, equipeInsertPoint.Y, 0), new Scale3d(1), 0); //插入监控单元图块
                            ...
                            registGouChaoInsertPoint.Add(new Point3d(equipeInsertPoint.X + marginEqui * numInsert - 1, equipeInsertPoint.Y, 0));  //绘制两条红线用于表示基站至信号电缆槽的挖沟
                            DBText equipeText = (DBText)drawF.insertText(StationToBeDraw.location + " " + StationToBeDraw.name, new Point3d(innerStartPoint.X + 146 + (218 / (List_EquipmentInfor.Count + 1)) * numInsert + 20, innerStartPoint.Y + 89 + 7.5 / 2, 0), 3, TextHorizontalMode.TextLeft, fontStyleId);
                        ...
                        {
                            foreach(var equipe in List_EquipmentInfor)
                            {
                                if (equipe.distance == LiCheng_Type.Key&& equipe.type == LiCheng_Type.Value)//绘制设备
                                {
                                    ...
                                    //插入文字‘防灾控制箱’
                                    DBText ControlBoxText = (DBText)drawF.insertText("防灾控制箱", new Point3d(equipeInsertPoint.X + 1.4 + marginEqui * numInsert + 2, equipeInsertPoint.Y + 40 - 23.4 + 4, 0), 3, TextHorizontalMode.TextLeft, fontStyleId);


                                    #region 文字：设备名称，线缆类型，方向及长度
                                    #region 设备名字
                                    //于下方表格中添加入设备文字信息
                                    DBText equipeText = (DBText)drawF.insertText(equipe.name + " " + equipe.location, new Point3d(innerStartPoint.X + 146 + (218 / (List_EquipmentInfor.Count + 1)) * numInsert, innerStartPoint.Y + 89 - 7.5 / 2, 0), 3, TextHorizontalMode.TextLeft, fontStyleId);
                                    ...
                                    Point3d endP = new Point3d(equipeInsertPoint.X + 1.4 + marginEqui * numInsert + 36, equipeInsertPoint.Y + 40 - 45.4 + 4, 0);
                                    Point3d startP = new Point3d(equipeInsertPoint.X + marginEqui * numInsert, equipeInsertPoint.Y + 40 - 45.4 + 4 - 12.07, 0);
                                    listP.Add(endP);
                                    listP.Add(new Point3d(endP.X - 31, endP.Y, 0));
                                    listP.Add(startP);
                    ...
                    #region 工程、设备数量表
                    int Num_DiXian = List_EquipmentInfor.Count * 10;  //地线
                    int Num_ChenDuan = List_EquipmentInfor.Count * 2; //电缆成端
                    int Num_GangGuan = 100;                           //钢管防护
                    int Num_Gou = 40;                                 //0.8电缆沟
                    int Num_Chao = List_EquipmentInfor.Count * 10;    //100*50电缆槽
                                                                      //LineLength
                    ...
                    Polyline[] mLinesGou = drawMutiLine(registGouChaoInsertPoint, new Point3d(innerStartPoint.X + 116, innerStartPoint.Y + 167, 0), 0.05, lineColor); //站点-电缆沟的红线
                    db2.AddToModelSpace(mLinesGou);

                    Polyline[] mLines2 = drawMutiLine(registEquipeInsertPoint);//防灾控制箱-设备 
...
*/
            ...
            foreach (var station in dictionyToFill)
            {
                if (station.Value.ToString().Contains("所") || station.Value.ToString().Contains("站"))
                {
                    if (!stTable.Rows.Contains(station.Key.ToUpper())) //如果datatable中不含有所亭项 
                    {
                        ...
                        stTable.Rows.Add(station.Key.ToUpper(), Loadvalue[0] , Loadvalue[1], Loadvalue[3], Loadvalue[2]);  //添加
                    }
        ...
        public void fileStationDataView(Control comp, System.Data.DataSet tableFill, List<string> listGridName, Dictionary<string, string> dictionyToFill, string tableName)
        {
            ...
            foreach (var station in dictionyToFill)
            {
                ...
                if (!table.Rows.Contains(station.Key.ToUpper())) //如果datatable中不含有所亭项 
                {
                    ...
                    table.Rows.Add(station.Key.ToUpper(), a[0], a[1], a[3], a[2]);  //添加
                }
        ...
        /// <summary>
        /// 确认用户在tab页面录入的数据，确认datagridview中的数据变更
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void B_Apply_DataGridView_Change_Click(object sender, EventArgs e)
        {
            string selectNode = tabControl1.SelectedTab.Text.ToString();  //选择的tab页
            XmlFunction xF = new XmlFunction();

            if (selectNode.Contains("风"))  //如果选择的是风速计tab
            {
                try
                {
                    if (MessageBox.Show("确认要保存风速计列表么", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        ...
                        InfoWindPoint.Clear();//清除所有的字典项
                        foreach (var item in WindTable.Rows)
                        {
                            DataRow rowItem = (DataRow)item;
                            //itemarray是一个数组，0-4分别是 里程、名称、类型、里程换算成数字、相对位置
                            ...
                            string rPosition=rowItem.ItemArray[3].ToString(); //相对位置
                            ...
                            if (location != "")
                            {
                                ...
                                List<string> listLoc = new List<string>();
                                //pF.isExMatch(location, @"^([A-Z]+)(\d+)\+(\d{0,4})$", out listLoc);
                                double distance = System.Convert.ToDouble(rowItem.ItemArray[4].ToString());//Int32.Parse(listLoc[1]) * 1000 + Int32.Parse(listLoc[2]);

                                //如果字典里没有这个项则添加到字典中
                                //if (ro)
                                if (!InfoWindPoint.ContainsKey(location))
                                {
                                    //InfoWindPoint.Add(location, name + ",风速计," + rPosition + "," + distance);//添加到字典
                                    InfoWindPoint.Add(location, name + ",风速计," + distance + "," + rPosition);//添加到字典
                                }
                        ...
                        //对字典进行排序
                        InfoWindPoint = LocationToIntToString(InfoWindPoint);
                        ...
                        foreach (KeyValuePair<string,string> item in InfoWindPoint)
                        {
                            string[] temp;
                            temp = item.Value.ToString().Split(new char[] { ',' });
                            tempDic.Add(item.Key, temp[0] + "," + temp[1] + "," + temp[3] + "," + temp[2]);
                        }

                        xF.addWayPointNode(xmlFilePath + "\\setting.xml", "WindPointLists", "WindPoints", tempDic);
                ...
                try
                {
                    if (MessageBox.Show("确认要保存雨量计列表么", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        ...
                        foreach (var item in RainTable.Rows)
                        {
                            ...
                            string rPosition = rowItem.ItemArray[4].ToString(); //相对位置
                            ...
                            if (location != "")
                            {
                                ...
                                {
                                    InfoRainPoint.Add(location, name + ",雨量计," + distance + "," + rPosition);//添加到字典
                        ...
                        InfoRainPoint = LocationToIntToString(InfoRainPoint);
                        ...
                        Dictionary<string, string> tempDic = new Dictionary<string, string>();
                        foreach (KeyValuePair<string, string> item in InfoRainPoint)
                        {
                            string[] temp;
                            temp = item.Value.ToString().Split(new char[] { ',' });
                            tempDic.Add(item.Key, temp[0] + "," + temp[1] + "," + temp[3] + "," + temp[2]);
                        ...
                        xF.addWayPointNode(xmlFilePath + "\\setting.xml", "RainPointLists", "RainPoints", tempDic);
                    ...
                    if (MessageBox.Show("确认要保存雪深计列表么", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        ...
                        foreach (var item in SnowTable.Rows)
                        {
                            ...
                            if (location != "")
                            {
                                ...
                                {
                                    InfoSnowPoint.Add(location, name + ",雪深计," + distance + "," + rPosition);//添加到字典
                        ...
                        InfoSnowPoint = LocationToIntToString(InfoSnowPoint);
                        ...
                        Dictionary<string, string> tempDic = new Dictionary<string, string>();
                        foreach (KeyValuePair<string, string> item in InfoSnowPoint)
                        {
                            string[] temp;
                            temp = item.Value.ToString().Split(new char[] { ',' });
                            tempDic.Add(item.Key, temp[0] + "," + temp[1] + "," + temp[3] + "," + temp[2]);
                        ...
                        xF.addWayPointNode(xmlFilePath + "\\setting.xml", "SnowPointLists", "SnowPoints", tempDic);
                    }
                }
                catch (System.Exception ee)
                {
        ...
        public Dictionary<string, string> LocationToIntToString(Dictionary<string, string> origDic)
        {
            ...
            foreach (var pair in SortNumdic)
            {
                ...
                Sortdic.Add(location[0], location[1] + "," + location[2] + "," + location[3] + "," + location[4]);
            }
        ...
        public void DrawSystemPicture(Point3d insertPoint, List<ClassStruct.StationPoint> dictRailStation, List<ClassStruct.StationPoint> StationList, List<ClassStruct.ConnectionAndLine> List_ConnectionAndLine,ObjectId FontId)
        {
            ...
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                ...
                {
                    MessageBox.Show("错误" + ee.ToString(), "错误", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
